...
export default function ProductGroups() {
  ...
  const [deletingGroup, setDeletingGroup] = useState<Category | null>(null);
  ...
  const openEditDialog = (group: Category) => {
  ...
  const handleDuplicate = async (group: Category) => {
    try {
      ...
      toast({
        title: 'Lỗi',
        description: error.message || 'Không thể nhân bản nhóm',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({
        title: 'Lỗi',
        description: error.message || 'Không thể lưu nhóm',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteCategory.mutateAsync(deletingGroup.id);
  ...
  const columns: Column<Category>[] = [
    ...
    },
    {
      ...
      render: (group) => (
        <Badge
          variant="outline"
          className={group.status === 'active' ? 'badge-success' : 'badge-warning'}
        >
      ...
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>Điền thông tin chi tiết cho nhóm sản phẩm</DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 py-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <Button type="button" variant="outline" size="sm" onClick={addConfigKey}>
              ...
              <div className="space-y-2 p-4 bg-muted/50 rounded-lg">
                {Object.entries(formData.configTemplate).map(([key, value]) => (
                  <div key={key} className="flex items-center gap-2">
                    <Input value={key} disabled className="w-32 bg-background" />
                    <Input
          ...
          <DialogFooter>
            ...
            <Button onClick={handleSave}>{editingGroup ? 'Cập nhật' : 'Tạo mới'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
