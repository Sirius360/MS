...
import { ManualDateTimePicker } from '@/components/ui/ManualDateTimePicker';
...
import { ArrowLeft, Plus, Trash2, Grid3X3, Search, Save, CheckCircle } from 'lucide-react';
...
export default function ImportCreate() {
  ...
  // Update item
  const updateItem = useCallback((id: string, field: keyof ImportItem, value: number) => {
  ...
  const handleCreateSupplier = async () => {
    ...
    try {
      ...
      setNewSupplier({ code: '', name: '', phone: '', address: '', notes: '' });
  ...
  const prepareOrderData = () => ({
    ...
    other_fee: otherCost,
    note,
    received_at: importDate.toISOString(),
  });
  ...
  // Complete import (create purchase receipt - backend handles inventory)
  const handleComplete = async () => {
    ...
    for (const item of items) {
      if (item.quantity <= 0) {
        toast({
          title: 'Lỗi',
          description: `Số lượng phải > 0: ${item.product_name}`,
          variant: 'destructive',
        });
        return;
      }
      if (item.unit_price < 0) {
        toast({
          title: 'Lỗi',
          description: `Đơn giá không được âm: ${item.product_name}`,
          variant: 'destructive',
        });
        return;
      }
    ...
    setIsSaving(true);
    try {
      // Create purchase receipt (backend automatically creates inventory transactions)
      await createPurchaseOrder.mutateAsync(prepareOrderData());
          ...
          <div className="col-span-2 space-y-4">
            ...
            <div className="flex items-center gap-3 bg-card rounded-lg border p-4">
              ...
              <Button
                variant="outline"
                size="icon"
                onClick={() => setIsQuickCreateProductOpen(true)}
              >
          ...
          <div className="space-y-4">
            ...
            <div className="bg-card rounded-lg border p-4 space-y-4">
              ...
              <div>
                ...
                <Input value="(Tự động sinh)" disabled className="mt-2 bg-muted" />
            ...
            <div className="flex gap-2">
              ...
              <Button className="flex-1" onClick={handleComplete} disabled={isSaving}>
