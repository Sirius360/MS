...
interface GenerateCodeRequest {
  type: 'product' | 'supplier' | 'purchase_order' | 'sales_order' | 'customer';
}
...
serve(async (req) => {
  ...
  try {
    ...
    const { type, code, action } = body;

    // Validate code uniqueness
    if (action === 'validate' && code) {
      const request = body as ValidateCodeRequest;
      console.log(`[CodeGenerator] Validating code: ${request.code} for type: ${request.type}`);
      ...
      switch (request.type) {
        ...
        case 'sales_order':
          ...
          break;
        default:
          throw new Error(`Invalid code type: ${request.type}`);
      }
      ...
      return new Response(
        JSON.stringify({ success: true, exists: !!data }),
    ...
    switch (type) {
      ...
      case 'supplier':
        ...
        break;
      case 'customer':
        functionName = 'generate_customer_code';
        break;
      case 'purchase_order':
